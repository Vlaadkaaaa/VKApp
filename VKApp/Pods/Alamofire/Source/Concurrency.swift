...
public extension Request {
    ...
    ///
    /// - Parameter bufferingPolicy: `BufferingPolicy` that determines the stream's buffering behavior.`.unbounded` by
    /// default.
    ///
    ...
    func urlSessionTasks(
        bufferingPolicy: StreamOf<URLSessionTask>
    ...
    private func stream<T>(
        ...
        yielder: @escaping (StreamOf<T>.Continuation) -> Void
    ) -> StreamOf<T> {
...
public struct DataTask<Value> {
    ...
    fileprivate init(
        request: DataRequest,
        task: Task<DataResponse<Value, AFError>, Never>,
        shouldAutomaticallyCancel: Bool
    ) {
...
public extension DataRequest {
    ...
    ///   - emptyResponseCodes:        HTTP response codes for which empty responses are allowed. `[204, 205]` by
    /// default.
    ...
    ///   - emptyResponseCodes:        HTTP status codes for which empty responses are always valid. `[204, 205]` by
    /// default.
    ///   - emptyRequestMethods:       `HTTPMethod`s for which empty responses are always valid. `[.head]` by default.
    ///
    /// - Returns: The `DataTask`.
    func serializingDecodable<Value: Decodable>(
    ...
    private func dataTask<Value>(
        automaticallyCancelling shouldAutomaticallyCancel: Bool,
        forResponse onResponse: @escaping (@escaping (DataResponse<Value, AFError>) -> Void)
            -> Void
    )
        -> DataTask<Value>
    {
...
public struct DownloadTask<Value> {
    ...
    fileprivate init(
        ...
        task: Task<AFDownloadResponse<Value>, Never>,
        shouldAutomaticallyCancel: Bool
    ) {
        self.request = request
        self.task = task
...
public extension DownloadRequest {
    ...
    ///
    /// - Returns: The `DownloadTask`.
    func serializingDownloadedFileURL(automaticallyCancelling shouldAutomaticallyCancel: Bool = false)
        -> DownloadTask<URL>
    {
        serializingDownload(
            using: URLResponseSerializer(),
            automaticallyCancelling: shouldAutomaticallyCancel
        )
    }
    ...
    ///
    /// - Returns: The `DownloadTask`.
    func serializingDownload<Serializer: ResponseSerializer>(
        using serializer: Serializer,
        automaticallyCancelling shouldAutomaticallyCancel: Bool =
            false
    )
    ...
    ///
    /// - Returns: The `DownloadTask`.
    func serializingDownload<Serializer: DownloadResponseSerializerProtocol>(
        using serializer: Serializer,
        automaticallyCancelling shouldAutomaticallyCancel: Bool =
            false
    )
    ...
    private func downloadTask<Value>(
        automaticallyCancelling shouldAutomaticallyCancel: Bool,
        forResponse onResponse: @escaping (
            @escaping (DownloadResponse<Value, AFError>)
                -> Void
        ) -> Void
    )
        -> DownloadTask<Value>
...
public struct DataStreamTask {
    ...
    ///   - bufferingPolicy: `         BufferingPolicy` that determines the stream's buffering behavior.`.unbounded` by
    /// default.
    ///
    /// - Returns:                   The `Stream`.
    public func streamingData(
        automaticallyCancelling shouldAutomaticallyCancel: Bool = true,
        bufferingPolicy: Stream<Data, Never>.BufferingPolicy = .unbounded
    ) -> Stream<Data, Never> {
    ...
    /// default.
    /// - Returns:
    public func streamingStrings(
        automaticallyCancelling shouldAutomaticallyCancel: Bool = true,
        bufferingPolicy: Stream<String, Never>.BufferingPolicy = .unbounded
    ) -> Stream<String, Never> {
        createStream(automaticallyCancelling: shouldAutomaticallyCancel, bufferingPolicy: bufferingPolicy) { onStream in
    ...
    ///
    /// - Returns:            The `Stream`.
    public func streamingDecodables<T>(
    ...
    {
        streamingResponses(
            serializedUsing: DecodableStreamSerializer<T>(),
            automaticallyCancelling: shouldAutomaticallyCancel,
    ...
    ///
    /// - Returns:           The `Stream`.
    public func streamingResponses<Serializer: DataStreamSerializer>(
        serializedUsing serializer: Serializer,
        automaticallyCancelling shouldAutomaticallyCancel: Bool =
            true,
        bufferingPolicy: Stream<
    ...
    private func createStream<Success, Failure: Error>(
        automaticallyCancelling shouldAutomaticallyCancel: Bool = true,
        bufferingPolicy: Stream<Success, Failure>
            .BufferingPolicy = .unbounded,
        forResponse onResponse: @escaping (@escaping (
            DataStreamRequest
...
fileprivate extension DispatchQueue {
    static let singleEventQueue = DispatchQueue(
        label: "org.alamofire.concurrencySingleEventQueue",
