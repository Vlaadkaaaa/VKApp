...
open class RetryPolicy: RequestInterceptor {
    ...
    public static let defaultRetryableURLErrorCodes: Set<URLError.Code> =
        [ // [Security] App Transport Security disallowed a connection because there is no secure network connection.
            ...
            // [System] An app or app extension attempted to connect to a background session that is already connected
            // to a
            // process.
            ...
            // [Data] Content data received during a connection request could not be decoded for a known content
            // encoding.
            //   - [Disabled] Server is unlikely to modify the content encoding during a retry.
            // .cannotDecodeRawData,
            ...
            // [File System] A request for an FTP file resulted in the server responding that the file is not a plain
            // file,
            // but a directory.
            ...
            // [System] The attempted connection required activating a data context while roaming, but international
            // roaming
            // is disabled.
            //   - [Enabled] WiFi connection could be established during retry.
            ...
            // [Resource] A server reported that a URL has a non-zero content length, but terminated the network
            // connection
    ...
    open func retry(
        ...
        if request.retryCount < retryLimit, shouldRetry(request: request, dueTo: error) {
            completion(.retryWithDelay(
                pow(Double(exponentialBackoffBase), Double(request.retryCount)) *
...
public extension RequestInterceptor where Self == ConnectionLostRetryPolicy {
    ...
    static func connectionLostRetryPolicy(
        retryLimit: UInt = RetryPolicy.defaultRetryLimit,
        exponentialBackoffBase: UInt = RetryPolicy.defaultExponentialBackoffBase,
        exponentialBackoffScale: Double = RetryPolicy.defaultExponentialBackoffScale,
        retryableHTTPMethods: Set<HTTPMethod> = RetryPolicy.defaultRetryableHTTPMethods
